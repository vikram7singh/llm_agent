...
class Agent:
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                "name": "callAgent",
                "description": "When the user asks to finsih a milestone, call this function. Make sure the milestote provided is part of the plan in artifacts/plan.md",
            }
        }
    ]
    ...
    async def execute(self, message_history):
        ...
        if function_name:
            ...
            if function_name == "updateArtifact":
                ...
                imp_agent = ImplementationAgent(name="Implementation Agent", client=self.client, prompt=self.prompt, gen_kwargs=self.gen_kwargs)
                await imp_agent.execute(message_history)

                message_history.append({
                        "role": "system",
                        "content": f"The milestone was updated."
                    })
